  ...
  return (
    <aside className="flex h-screen w-64 shrink-0 flex-col bg-slate-900">
      ...
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-white/10">
        ...
        <div>
          <p className="text-sm font-semibold text-white leading-tight">Tavex</p>
          <p className="text-[10px] text-slate-400 leading-tight truncate max-w-[160px]">{organizacionNombre}</p>
        </div>
      </div>

      {/* Selector de cliente */}
      <div className="px-4 py-3 border-b border-white/10">
        <p className="mb-1.5 text-[10px] font-semibold tracking-widest text-slate-500 uppercase">Cliente activo</p>
        <form ref={formRef} action={seleccionarClienteAction}>
          <select
            name="clienteId"
            className="w-full rounded-lg border border-white/10 bg-slate-800 px-2.5 py-1.5 text-xs text-slate-200 disabled:opacity-60 focus:outline-none focus:ring-1 focus:ring-indigo-500"
          ...
          >
            <option value="">
              {clientes.length === 0 ? "Sin clientes" : "— Seleccionar —"}
            </option>
            {clientes.map((c) => (
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        ...
        {esAdmin && (
          <NavItem href="/app/usuarios" icon={<IconUsers />} label="Usuarios" />
        )}
